...
for _m in _memebers:
    locals()[
        _m.__name__
    ] = _m  # Assign to local namespace that can be exported, classes only have __name__, not name
...
class POSCAR:
    ...
    def from_cif(cls, path):
        ...
        with p.open("r", encoding="utf-8") as f:
            cif_str = f.read()
            poscar_content = sio._cif_str_to_poscar_str(
    ...
    def get_cell(self, loop=True):
        ...
        self._cell = serializer.CellData(
            sio.get_bz(
                path_pos=self._data.rec_basis, loop=loop, primitive=True
            ).to_dict()
    ...
    def rotate(self, angle_deg, axis_vec):
        return self.__class__(
            data=sio.rotate_poscar(self._data, angle_deg=angle_deg, axis_vec=axis_vec)
        )
    ...
    def repeat(self, n, direction):
        return self.__class__(
            data=sio.repeat_poscar(self._data, n=n, direction=direction)
        )
    ...
    def transform(self, transformation, zoom=2, tol=1e-2):
        return self.__class__(
            data=sio.transform_poscar(self._data, transformation, zoom=zoom, tol=tol)
        )
    ...
    def add_atoms(self, name, positions):
        return self.__class__(
            data=sio.add_atoms(self._data, name=name, positions=positions)
        )

    @_sub_doc(sio.convert_poscar, "- poscar_data")
    def convert(self, atoms_mapping, basis_factor):
        return self.__class__(
            data=sio.convert_poscar(
    ...
    def strain(self, strain_matrix):
        return self.__class__(
            data=sio.strain_poscar(self._data, strain_matrix=strain_matrix)
        )
    ...
    def bring_in_bz(self, kpoints, shift=0):
        ...
        return sio.kpoints2bz(
            self._bz, kpoints=kpoints, primitive=self._primitive, shift=shift
        )
...
class _BandsDosBase:
    def __init__(self, source):
        if not isinstance(source, vp.DataSource):
            raise TypeError(
                "`source` must be a subclass of `ipyvasp.parser.DataSource`."
            )
...
class DOS(_BandsDosBase):
    ...
    def iplot_dos_lines(
        ...
        **kwargs,
        ...
        return ip.iplot_dos_lines(energy - data.ezero, dos_arrays, labels, **kwargs)
