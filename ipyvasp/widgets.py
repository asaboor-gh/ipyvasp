...
__all__ = ['css_style', 'dark_colors', 'light_colors', 'simple_colors', 'default_colors', 'InputGui',
           'summarize', 'FilesWidget', 'VasprunApp', 'KpathWidget']
...
_kpath_instructions = Markdown('''
### Instructions for using KpathWidget
...
class KpathWidget(ipw.VBox):
    def __init__(self):
        super().__init__()
        self._fig = go.FigureWidget()
        self._sm = ipw.SelectMultiple(options = [('⋮',0),], layout = ipw.Layout(width = 'auto'))
        self._lab = ipw.Text(description='Labels',continuous_update = True)
        self._kpt = ipw.Text(description='KPOINT', continuous_update = False)
        self._add = ipw.Button(description = '➕ Point', tooltip = 'Add point to path')
        self._del = ipw.Button(description = '❌ Point', tooltip = 'Delete Selected Points')
        self._tsb = ipw.Button(description = 'Dark Plot', tooltip = 'Toggle Plot Theme')
        self._poscar = None
        self._kpoints = {}
        
        other_controls = [ipw.HBox([self._add, self._del, self._tsb],layout = ipw.Layout(min_height = '24px')), self._sm, self._lab, self._kpt]
        self._interact = FilesWidget().interactive(self._update_fig,[self._fig],other_controls)
    ...
    def usage_info(self):
        return display(_kpath_instructions)
    ...
    def get_kpoints(self):
        # no need to capture in ouput, outside function will do it
