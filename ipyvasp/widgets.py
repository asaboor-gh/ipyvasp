...
from ipywidgets import Layout,Button,Box,HBox,VBox,Dropdown,Text,Checkbox, Stack, SelectMultiple
...
class FilesWidget(VBox):
    ...
    def __init__(self, path : str = '.', glob :str = '*', exclude :str = None, on_file_changed = None) -> None:
        ...
        self._widgets = {
            ...
            'files' : Dropdown(options = [], description = 'File:', tooltip = 'Select a file from the list.'),
        }
    ...
    def interactive(self, func, other_widgets = None, other_controls = None, options = {'manual':False}, height = '90vh', **kwargs):
        ...
        output.layout = Layout(overflow = 'auto', max_height = '100%', width = '100%') # make output scrollable and avoid overflow
...
class _PropPicker(VBox):
    ...
    def _process(self, system_summary):
        if not hasattr(system_summary, 'orbs'):
            self.children = [ipw.HTML(f'❌ No projection data found from given summary!')]
            return None
        ...
        if set(['px','py','pz']).issubset(sorbs):
            orbs['p'] = range(1,4)
            orbs['px+py'] = [idx for idx,key in enumerate(sorbs) if key in ('px','py')]
...
class BandsWidget(VBox):
    ...
    def __init__(self, use_vaspout = False, height = '90vh', **file_widget_kwargs):
        ...
        self._tsd = Dropdown(description = 'Style', options = ['plotly_white','plotly_dark'])
...
class KpathWidget(VBox):
    ...
    def get_kpoints(self):
        ...
        keys = [idx for (_,idx) in self._sm.options if idx in self._kpoints] # order and existence is important
        kpts = [self._kpoints[k] for k in keys]
        LN = [lab.split('⋮')[1].strip().split() for (lab, idx) in self._sm.options if idx in keys]
        ...
        for idx,ln in enumerate(LN):
            if len(ln) == 2:
                ...
                raise ValueError('Label and number should be separated by space or only one of them should be present')
        return kpts
    ...
    def _add_label(self, change):
        with self._interact.output_widget:
            ...
            for idx, (_, lab) in enumerate(zip(self._sm.options, self._lab.value.split(','))):
